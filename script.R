...
# #Problem defenition: ---------------------------------------------------------------
#1 + 2: In the attached Word file

#Understanding the data
#1. In the attached Word file

#2.
# Initial plotting & correlation test -------------------------------------
#Target lable:

plot(as.factor(train$y), main = "Heart attack") 
# The target lable is balanced -> we excpect the model to predit around 
...
corrplot(cor(train)) 
# slope vs. oldpeak are a bit correlated -> Why? What does it mean?

cor(train$slope,train$oldpeak) 
# -0.58 )No meaning - slope is a categorial feature.

#What might have a correlation with the target lable:
cor(train$y,train$cp)
cor(train$y,train$thalach)
cor(train$y,train$exang)
cor(train$y,train$oldpeak)  


#no one is greather than |0.5| #Do we have any asumptions on what can influent it else wise?
#it's ok if noone by itself is highly correlated with the target - we are unaware of the combinations of features and how they are correlated with the target
...
grid.arrange(
  ...
  ggplot(grouped_data,aes(x=gender,group=type,fill=type)) + geom_histogram(alpha=0.5) + theme(legend.position="none") + ylab(NULL), # after age prediction
